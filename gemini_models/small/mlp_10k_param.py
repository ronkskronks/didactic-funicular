...
# using NumPy, designed to have approximately 10,000 trainable parameters.
# It includes functionality to save and load models, allowing for continued training.
...
#    - Each of the 2000 hidden neurons receives input from 2 input neurons.
#    - Number of weights = 2000 * 2 = 4000
# 2. Biases for Hidden Layer 1:
#    - Each of the 2000 hidden neurons has its own bias.
#    - Number of biases = 2000
# 3. Weights connecting Hidden Layer 1 to Output Layer:
#    - Each of the 2 output neurons receives input from 2000 hidden neurons.
#    - Number of weights = 2 * 2000 = 4000
# 4. Biases for Output Layer:
#    - Each of the 2 output neurons has its own bias.
#    - Number of biases = 2
#
# Total Parameters = (4000 + 2000) + (4000 + 2) = 6000 + 4002 = 10002 parameters.
# This is very close to the requested 10,000 parameters.
...
class SimpleMLP:
    ...
    def __init__(self, input_size, hidden_size, output_size, learning_rate=0.01):
        ...
        # This will be 4000 + 2000 + 4000 + 2 = 10002 for the chosen architecture.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train or continue training a Simple MLP model.")
    parser.add_argument('--model_file', type=str, default="mlp_10k_param_model.pkl",
    ...
    # Define the architecture: Input (2) -> Hidden (2000) -> Output (2)
    input_dim = 2
    hidden_dim = 2000
    output_dim = 2
    ...
    if os.path.exists(args.model_file) and not args.new_model:
        ...
        try:
            model = SimpleMLP.load_model(args.model_file)
            # Ensure loaded model matches expected architecture (optional, but good practice)
            if not (model.input_size == input_dim and \
                    model.hidden_size == hidden_dim and \
                    model.output_size == output_dim):
                print("Warning: Loaded model architecture does not match expected. Creating new model.")
                model = None # Force new model creation
    ...
    if model is None:
        ...
        model = SimpleMLP(input_dim, hidden_dim, output_dim, args.learning_rate)
    ...
    model.train(X_train, y_train, args.epochs)
